#Color Theme Maker
import tkinter
from tkinter import BOTH, IntVar, DISABLED, NORMAL, filedialog
...
def update_color():
    """UPdate the inner color box based on the slider values.  Display tuple and hex values of the current color"""
    #Update the backgoround color of the inner color box
    inner_color_box.config(bg="#" + red_value + green_value + blue_value)
...
def store_color():
    ...
    #Update the widgets for the stored color
    i = stored_color.get()
    recall_button[i].config(command=lambda:set_color(stored_red, stored_green, stored_blue), state=NORMAL)
    new_color_tuple[i].config(text='(' + red + '),' + '(' + green + '),' + '(' + blue + ')')
    new_color_hex[i].config(text='#' + red_value + green_value + blue_value)
    new_color_box[i].config(bg="#"+ red_value + green_value + blue_value)

    #Update the dict stored_colors with the new color tuple and hex values
    stored_colors[stored_color.get()] = [new_color_tuple[i].cget("text"), new_color_hex[i].cget("text")]
...
#Make the inner color box smaller than the outer due to ipadx and ipday on the outer color box
inner_color_box = tkinter.Label(input_frame, bg="#000000", height=6, width=15)
inner_color_box.grid(row=1, column=3, columnspan=2, padx=35, pady=10)
...
for i in range(6):
    radio[i] = tkinter.Radiobutton(output_frame, variable=stored_color, value=i)
    radio[i].grid(row=i, column=0, sticky='W')

    recall_button[i] = tkinter.Button(output_frame, text="Recall Color", state=DISABLED)
    new_color_tuple[i] = tkinter.Label(output_frame, text="(255), (255), (255)")
    new_color_hex[i] = tkinter.Label(output_frame, text="#ffffff")
    new_color_black_box[i] = tkinter.Label(output_frame, bg="black", width=3, height=1)
    new_color_box[i] = tkinter.Label(output_frame, bg='white', width=3, height=1)

    recall_button[i].grid(row=i, column=1, padx=20)
    new_color_tuple[i].grid(row=i, column=2, padx=20)
    new_color_hex[i].grid(row=i, column=3, padx=20)
    new_color_black_box[i].grid(row=i, column=4, pady=2, ipadx=5, ipady=5)
    new_color_box[i].grid(row=i, column=4)
